...
value_type BeamCKYParser::beam_prune(std::unordered_map<int, State> &beamstep)
{
    ...
    for (auto &item : beamstep)
    {
        ...
        if ((k >= 0) && (bestC[k].score == VALUE_MIN))
            ...
            newscore = (k >= 0 ? bestC[k].score : 0) + cand.score;
                          ...
                          std::vector<std::pair<value_type, int>> &sorted_stepM)
{
    ...
    if (threshold == VALUE_MIN)
    {
        ...
        for (auto &item : beamstep)
        {
            ...
            if ((use_constraints) && (k >= 0) && (bestC[k].score == VALUE_MIN))
                newscore = cand.score;
            else
                newscore = (k >= 0 ? bestC[k].score : 0) + cand.score;
...
bool check_pairable_ij(vector<int> &SS_fast_i, vector<int> &SS_fast_j, float **ribo, vector<vector<ribo_state>> &pscore, int i, int j)
{ // it is hc_decompose  = fc->hc->mx[n * i + j]; in mfe.c
...
BeamCKYParser::DecoderResult BeamCKYParser::parse_alifold(std::vector<std::string> &MSA, float **ribo, vector<vector<ribo_state>> &pscore, vector<vector<int>> &a2s_fast, vector<vector<int>> &s5_fast, vector<vector<int>> &s3_fast, vector<vector<int>> &SS_fast, vector<float> &smart_gap)
{
    ...
    if (seq_length > 0)
        bestC[0].set(-em.score_external_unpaired(0, 0), MANNER_C_eq_C_plus_U);
    if (seq_length > 1)
        bestC[1].set(-em.score_external_unpaired(0, 1), MANNER_C_eq_C_plus_U);
    ...
    for (int j = 0; j < seq_length; ++j)
    {
            ...
            if (smart_gap[j] - smart_gap[j - 1] > smart_gap_threshold)
            {
                ...
                else
                {
                    ...
                    for (int s = 0; s < MSA.size(); s++)
                    {
                        ...
                        while (jnext_temp - j < 4 && jnext_temp != -1)
                            jnext_temp = next_pair_MSA[s][SS_j[s]][jnext_temp];
                ...
                while (1)
                {
                    if (jnext == -1)
                        break;
                    if (check_pairable_ij(SS_j, SS_fast[jnext], ribo, pscore, j, jnext))
                        break;
                    ...
                    else
                    {
                        ...
                        for (int s = 0; s < MSA.size(); s++)
                        {
                            ...
                            if (jnext_temp != -1)
                                jnext_min = min(jnext_min, jnext_temp);
                        }

                        if (jnext_min != jnext_org)
                            jnext = jnext_min;
                        else
                            jnext = -1;
                ...
                if (jnext != -1)
                {
                    ...
                    for (int s = 0; s < n_seq; s++)
                    {
                        ...
                        if (u < 3)
                            ...
                            newscore += -em.score_hairpin(0, u + 1, new_nucj, new_nucj1, new_nucjnext_1, new_nucjnext, tetra_hex_tri);
                    }
            ...
            }
            {
                ...
                for (auto &item : keys)
                {
                    ...
                    while (1)
                    {
                        ...
                        if (jnext == -1)
                            break;
                        if (check_pairable_ij(SS_i, SS_fast[jnext], ribo, pscore, i, jnext))
                            break;
                    ...
                    if (jnext != -1)
                    {
                        ...
                        for (int s = 0; s < n_seq; s++)
                        {
                            ...
                            if (u < 3)
                                ...
                                newscore += -em.score_hairpin(0, u + 1, new_nuci, new_nuci1, new_nucjnext_1, new_nucjnext, tetra_hex_tri);
                        }
        ...
        // beam of Multi
        {
            if (beam > 0 && beamstepMulti.size() > beam)
            ...
            for (auto &item : keys)
            {
...

                {
                    ...
                    for (int s = 0; s < n_seq; s++)
                    {
                        ...
                        new_nucj_1 = s5_j[s];
                        new_nucj = SS_j[s];
                        newscore += -em.score_multi(-1, -1, new_nuci, new_nuci1, new_nucj_1, new_nucj, -1);
                    }
            ...
            for (auto &item : keys)
            {
                ...
                if (i > 0 && j < seq_length - 1)
                {
                    ...
                    for (int s = 0; s < n_seq; s++)
                    {
                        ...
                        newscore += -em.score_M1(-1, -1, -1, new_nuci_1, new_nuci, new_nucj, new_nucj1, -1); // no position information needed
                    }
                ...
                if (!use_cube_pruning)
                {
                    ...
                    if (k > 0 && !bestM[k].empty())
                    {
                        ...
                        for (int s = 0; s < n_seq; s++)
                        {
                            ...
                            M1_score += -em.score_M1(-1, -1, -1, new_nuci_1, new_nuci, new_nucj, new_nucj1, -1);
                        }
                        // candidate list
                ...
                // 4. C = C + P
                {
                    ...
                    if (k >= 0)
                    {
                        ...
                        if (prefix_C.manner != MANNER_NONE)
                        {
                            ...
                            for (int s = 0; s < n_seq; s++)
                            {
                                ...
                                newscore += -em.score_external_paired(-1, -1, new_nuck, new_nuck1, new_nucj, new_nucj1, -1);
                            }
                    ...
                    else
                    {
                        ...
                        for (int s = 0; s < MSA.size(); s++)
                        {
                            ...
                            new_nucj = SS_j[s];
                            new_nucj1 = (a2s_j[s] < a2s_seq_length_1[s]) ? s3_j[s] : -1; // external.c line 1165, weird
                            newscore += -em.score_external_paired(0, j, -1, new_nuck1, new_nucj, new_nucj1, -1);
                        }
                            ...
                            if (check_pairable_ij(SS_p, SS_q, ribo, pscore, p, q))
                            {
                                ...
                                if (p == i - 1 && q == j + 1)
                                {
                                    ...
                                    for (int s = 0; s < n_seq; s++)
                                    {
                                        ...
                                        newscore += -em.score_single_alifold(0, 0, type, tt2[s], nucp1, nucq_1, nuci_1, nucj1); // internal.c line 476, left, right gaps are all 0
                                    }
                                ...
                                else
                                {
                                    ...
                                    for (int s = 0; s < n_seq; s++)
                                    {
                                        ...
                                        type = NUM_TO_PAIR(nucp, nucq);
                                        newscore += -em.score_single_alifold(u1_local, u2_local, type, tt2[s], nucp1, nucq_1, nuci_1, nucj1);
                                    }
        ...
        // beam of M2
        {
            if (beam > 0 && beamstepM2.size() > beam)
                ...
                // 1. multi-loop
                {
                    ...
                    for (int p = i - 1; ((smart_i - smart_gap[p]) <= 2 * SINGLE_MAX_LEN) and p >= 0; --p)
                    {
                        ...
                        while (1)
                        {
                            ...
                            if (q == -1)
                                break;
                            if (check_pairable_ij(SS_p, SS_fast[q], ribo, pscore, p, q))
                                break;
        ...
        // beam of M
        {
            ...
            if (beam > 0 && beamstepM.size() > beam)
                threshold = beam_prune(beamstepM);
